    ...
    'DocPageParams': {
        'struct': {
            ...
            'members': [
                ...
                {
                    'name': 'url',
                    'doc': ['The JSON type model resource URL'],
                    'type': {'builtin': 'string'},
                    'attr': {'lenGT': 0},
                    'optional': true
                }
 ...
 *
 * @property {?string} typeModel - The type model object
 * @property {?string} typeModelURL - The JSON type model resource URL
...
export class DocPage {
    ...
    constructor(typeModel = null) {
        this.typeModel = typeModel !== null && typeof typeModel !== 'string' ? chisel.validateTypeModel(typeModel) : null;
        this.typeModelURL = typeModel !== null && typeof typeModel === 'string' ? typeModel : null;
    ...
    render() {
        ...
        // Type model loaded?
        const typeModelURL = this.config.url !== null ? this.config.url : this.typeModelURL;
        if (this.typeModel !== null) {
            this.renderTypeModelPage();
            ...
            window.fetch(typeModelURL).
                ...
                then((typeModel) => {
                    this.typeModel = chisel.validateTypeModel(typeModel);
                    this.renderTypeModelPage();
                }).catch(({message}) => {
                    DocPage.renderErrorPage(message);
                });
    ...
    renderTypeModelPage() {
        ...
        if (this.config.name !== null) {
            // Unknown type?
            if (!(this.config.name in this.typeModel.types)) {
                DocPage.renderErrorPage(`Unknown type name '${this.config.name}'`);
            } else {
                this.renderRequestPage({'name': this.config.name, 'types': this.typeModel.types});
            }
            return;
        }
        ...
        const userTypesSorted = Object.entries(this.typeModel.types).sort();
