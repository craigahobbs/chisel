...
export function render(parent, elems, clear) {
    if (clear) {
        parent.innerHTML = '';
    }
...
export function elem(tag, attrsOrElems, elems) {
    let attrs = isObject(attrsOrElems) ? attrsOrElems : undefined;
    return {
        tag: tag,
        attrs: attrs || {},
...
export function xhr(method, url, async, args) {
    args = args || {};
    let xhr_ = new XMLHttpRequest();
    xhr_.open(method, href(null, args.params, url), async);
